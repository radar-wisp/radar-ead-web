...
var Admin = (() => {
    ...
    inp.addEventListener('input', () => {
      ...
      res.innerHTML = [
        cs.length ? `<div class="sr-group">Cursos</div>` + cs.slice(0,4).map(c =>
          `<div class="sr-item" onclick="Admin._searchGo('cursos','${c.id}')"><span>${c.emoji||'📚'}</span><div><div class="sr-title">${x(c.titulo)}</div><div class="sr-sub">${statusBadge(c.status||'rascunho')}</div></div></div>`).join('') : '',
        as.length ? `<div class="sr-group">Colaboradores</div>` + as.slice(0,3).map(a =>
          `<div class="sr-item" onclick="Admin._searchGo('colaboradores')"><span>👤</span><div><div class="sr-title">${x(a.nome)}</div><div class="sr-sub" style="color:var(--t3);font-size:.75rem">${x(a.email)}</div></div></div>`).join('') : '',
      ].join('');
  ...
  function dashboard() {
    ...
    q('#ds-pendencias').innerHTML = pends.length
      ...
      : `<div class="pend-ok"><span>✅</span> Tudo em ordem! Nenhuma pendência.</div>`;
    ...
    if (!allCursos.length) { tbody.innerHTML = tdEmpty(4,'Nenhum curso cadastrado'); }
      ...
      return `<tr>
        <td><div style="display:flex;align-items:center;gap:10px"><div class="curso-mini-icon">${c.emoji||'📚'}</div><div><div style="font-weight:600;font-size:.86rem">${x(c.titulo)}</div><div style="color:var(--t3);font-size:.75rem">${mods} módulos · ${aulas} aulas${c.carga?' · '+c.carga+'h':''}</div></div></div></td>
        ...
        <td><div class="action-menu-wrap"><button class="btn btn-sm btn-ghost action-menu-btn" onclick="Admin._toggleMenu(this)">Ações ▾</button><div class="action-menu">
          <button onclick="Admin.openModalCurso('${c.id}');Admin._closeMenus()">✏️ Editar</button>
          ${status!=='publicado'?`<button onclick="Admin.publicar('${c.id}');Admin._closeMenus()">🚀 Publicar</button>`:`<button onclick="Admin.arquivar('${c.id}');Admin._closeMenus()">📦 Arquivar</button>`}
          <button onclick="Admin.duplicarCurso('${c.id}');Admin._closeMenus()">⧉ Duplicar</button>
          <button onclick="Admin.go('materiais');Admin._closeMenus()">📁 Materiais</button>
          <button onclick="Admin.goAcessos('${c.id}');Admin._closeMenus()">🔒 Acessos</button>
          <button class="danger" onclick="Admin.excluirCurso('${c.id}');Admin._closeMenus()">🗑️ Excluir</button>
        </div></div></td>
      </tr>`;
    }).join('');
  ...
  function renderCursosList() {
    ...
    if (!lista.length) { wrap.innerHTML = `<div class="empty"><div class="ei">📭</div><p>Nenhum curso. Crie o primeiro!</p></div>`; return; }
    wrap.innerHTML = lista.map(c => {
      ...
      return `<div class="curso-card">
        <div class="curso-card-left"><div class="curso-emoji">${c.emoji||'📚'}</div><div class="curso-info"><div class="curso-titulo">${x(c.titulo)}</div><div class="curso-meta">${mods} módulos · ${aulas} aulas · ${mats} materiais · ${c.carga||0}h${c.validadeAte?' · válido até '+fmtDate(c.validadeAte):''}</div></div></div>
        <div class="curso-card-right">
          ...
          <div class="action-menu-wrap"><button class="btn btn-sm btn-ghost action-menu-btn" onclick="Admin._toggleMenu(this)">Ações ▾</button><div class="action-menu">
            <button onclick="Admin.openModalCurso('${c.id}');Admin._closeMenus()">✏️ Editar</button>
            ${status!=='publicado'?`<button onclick="Admin.publicar('${c.id}');Admin._closeMenus()">🚀 Publicar</button>`:`<button onclick="Admin.arquivar('${c.id}');Admin._closeMenus()">📦 Arquivar</button>`}
            <button onclick="Admin.duplicarCurso('${c.id}');Admin._closeMenus()">⧉ Duplicar</button>
            <button onclick="Admin.go('materiais');Admin._closeMenus()">📁 Materiais</button>
  ...
  function renderModulosEditor(cursoId) {
    ...
    wrap.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px"><strong style="font-size:.85rem">Módulos e Aulas</strong><button class="btn btn-sm btn-soft" onclick="Admin.addModulo('${cursoId}')">+ Módulo</button></div>
      ${modulos.length ? modulos.map(m => {
        ...
        return `<div class="mod-bloco"><div class="mod-header"><span>${m.ordem}. ${x(m.titulo)}</span><div style="display:flex;gap:6px"><button class="btn btn-sm btn-soft" onclick="Admin.addAula('${m.id}','${cursoId}')">+ Aula</button><button class="btn btn-sm btn-danger" onclick="Admin.delModulo('${m.id}','${cursoId}')">🗑️</button></div></div>
          ${aulas.map(a=>`<div class="aula-row"><span class="badge ${tipoBadge(a.tipo)}">${a.tipo}</span><span>${x(a.titulo)}</span><span style="color:var(--t3);font-size:.76rem;margin-left:auto">${a.duracao||0}min</span><button class="btn btn-sm btn-danger" onclick="Admin.delAula('${a.id}','${m.id}','${cursoId}')">🗑️</button></div>`).join('')}
        </div>`;
  ...
  function publicacao() {
    ...
    wrap.innerHTML=cursos.length?cursos.map(c=>{const aulas=Storage.Aulas.totalPorCurso(c.id),mods=Storage.Modulos.listarPorCurso(c.id).length,rest=Storage.Restricoes.porCurso(c.id).length,status=c.status||'rascunho',exp=c.validadeAte&&new Date(c.validadeAte)<agora,pronto=aulas>0&&mods>0;return`<div class="pub-card"><div class="pub-card-left"><div style="font-size:1.6rem">${c.emoji||'📚'}</div><div><div style="font-weight:600;font-size:.92rem">${x(c.titulo)}</div><div style="color:var(--t3);font-size:.78rem;margin-top:2px">${mods} módulos · ${aulas} aulas · ${rest?rest+' restrição(ões)':'Acesso livre'}${c.validadeAte?' · Validade: '+fmtDate(c.validadeAte):''}</div></div></div><div class="pub-card-right">${statusBadge(status)}${exp?'<span class="badge badge-red">Expirado</span>':''}${!pronto&&status==='rascunho'?'<span class="badge badge-amber">Sem conteúdo</span>':''}${status==='rascunho'?`<button class="btn btn-sm btn-primary" onclick="Admin.publicar('${c.id}')" ${!pronto?'disabled':''}>▶ Publicar</button>`:status==='publicado'?`<button class="btn btn-sm btn-ghost" onclick="Admin.arquivar('${c.id}')">⏸ Arquivar</button><button class="btn btn-sm btn-soft" onclick="Admin.publicar('${c.id}')">↺ Republicar</button>`:`<button class="btn btn-sm btn-primary" onclick="Admin.publicar('${c.id}')">▶ Reativar</button>`}<button class="btn btn-sm btn-ghost" onclick="Admin.openValidade('${c.id}')">📅 Validade</button><button class="btn btn-sm btn-ghost" onclick="Admin.duplicarCurso('${c.id}')">⧉ Duplicar</button></div></div>`;}).join(''):`<div class="empty"><div class="ei">📭</div><p>Nenhum curso cadastrado.</p></div>`;
  }
  ...
  function bindModals() {
    ...
    q('#mc-form').onsubmit=e=>{e.preventDefault();const d={titulo:q('#mc-titulo').value.trim(),descricao:q('#mc-desc').value.trim(),emoji:q('#mc-emoji').value.trim()||'📚',carga:parseInt(q('#mc-carga').value)||0,status:q('#mc-status').value||'rascunho',validadeAte:q('#mc-validade').value?new Date(q('#mc-validade').value).toISOString():null};if(!d.titulo)return;if(modal.ctx.cursoId){Storage.Cursos.atualizar(modal.ctx.cursoId,d);toast('Atualizado!','s');}else{const n=Storage.Cursos.criar(d);modal.ctx.cursoId=n.id;renderModulosEditor(n.id);toast('Criado!','s');}renderCursosList();};
  ...
  function statusBadge(status){
    const cfg={publicado:{cls:'badge-green',label:'● Publicado'},rascunho:{cls:'badge-amber',label:'✎ Rascunho'},revisao:{cls:'badge-blue',label:'◎ Revisão'},arquivado:{cls:'badge-gray',label:'▣ Arquivado'},expirado:{cls:'badge-red',label:'✕ Expirado'}};
  ...
  function toast(msg,tipo='i'){ const s=document.getElementById('toasts'),el=document.createElement('div');el.className=`toast ${tipo}`;el.innerHTML=`<span>${{s:'✅',e:'❌',i:'ℹ️'}[tipo]||'ℹ️'}</span><span>${msg}</span>`;s.appendChild(el);setTimeout(()=>el.remove(),3000); }
